...
public class EnemySpawner extends GameObject{
	...
	public boolean properties() {
		...
		if (mods.getSelectedIndex() == 1) {
			default_enemy = JOptionPane.showInputDialog(LevelEditor.frame, "Enemy Type [full path]");
			this.enemy_type = default_enemy;
		}
		if (mods.getSelectedIndex() == 2) {
			default_enemy_count = Integer.parseInt(JOptionPane.showInputDialog(LevelEditor.frame, "How Many Enemies to Spawn?"));
			this.enemy_count = default_enemy_count;
		}
		if (mods.getSelectedIndex() == 3) {
			default_respawn = JOptionPane.showInputDialog(LevelEditor.frame, "Respawn Enemies? [yes/no]").equals("yes");
