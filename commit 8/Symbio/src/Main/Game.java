...
public class Game {	
	...
	public static void game() {
		...
		do {
			...
			//draw next frame
			cam.draw_view(Start.gs[Start.count%2], delta_time);
		...
		Start.frame.addKeyListener(new KeyListener() {
			...
			public void keyPressed(KeyEvent e) {
				...
				if (e.getKeyCode() == Settings.slash_key) {
					if (player.slash_wait_count == 0 && !player.slash_held && !player.slashing && player.slash_count == 0) {
						player.start_slash();
					}
					else player.slash_buffer = 6;
				}
				//shift to grab
