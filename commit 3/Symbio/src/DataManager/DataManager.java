...
public class DataManager {
	...
	//ALL TOGETHER
	public static void save(Room room, int room_num, int grid_size, boolean delete_room) {
		try {
	...
	public static Room load_room(int room, int grid_size) {
		try {
			...
			return read_room(read, grid_size, true);
	...
	public static Room read_room(BufferedReader read, int grid_size, boolean load_contents) {
		Room out = new Room();
		try {
			...
			for (int x = 0; x<num; x++) {
				...
				out.objects[x] = GameObject.create_game_object(sep, x);
	...
	public static Rectangle[] load_rects(int grid_size, int room_to_tile) {
		...
		try {
			...
			for (int x = 0; x<rects.length; x++) {
				rects[x] = read_room(read, grid_size, false);
