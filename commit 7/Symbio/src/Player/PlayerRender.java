...
public class PlayerRender {
	...
	public static void draw_player(Graphics g, JPanel pane, double xin, double yin, String location) {
		...
		anim.play(false, Vector2.zero, Game.player.last_dir == -1, g, pane, xin, yin, location);
		
		
		if (Game.player.slashing) {
			new Rectangle(Game.player.pos.x + Game.player.slash_dir.x, Game.player.pos.y + Game.player.slash_dir.y, (Game.player.slash_dir.x != 0 ? 1 : 0) * Game.player.slash_bounds.x + (Game.player.slash_dir.x == 0 ? 1: 0) * Game.player.slash_bounds.y, (Game.player.slash_dir.y != 0 ? 1.5 : 0) * Game.player.slash_bounds.x + (Game.player.slash_dir.y == 0 ? 1: 0) * Game.player.slash_bounds.y).draw(g, Start.pane, Game.cam.pos.x, Game.cam.pos.y, "game");
		}
		
		if (Game.player.dash_num == 1) new Vector2(Game.player.pos.x, Game.player.pos.y + 10).draw_node(g, Start.pane, Game.cam.pos.x, Game.cam.pos.y, "game", Color.green);
		if (Game.player.dash_num == 0) new Vector2(Game.player.pos.x, Game.player.pos.y + 10).draw_node(g, Start.pane, Game.cam.pos.x, Game.cam.pos.y, "game", Color.red);
	}
