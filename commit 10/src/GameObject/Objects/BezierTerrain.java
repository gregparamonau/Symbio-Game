...
public class BezierTerrain extends GameObject{
	...
	public void displace_entity(Rectangle in, int direction) {
		...
		for (int x = 0; x<this.shape.lines.length; x++) {
			if (!Rectangle.intersect_line(in, this.shape.lines[x].a, this.shape.lines[x].b)) continue;
			Line.disp_rect(this.shape.lines[x], in, 0);
		}
		
		in.pos.add(Vector2.sub(this.pos, this.last_pos));
	...
	public void draw_object(Graphics g, JPanel pane, double xin, double yin, String location) {
		...
		if (location.equals("game")) {
			...
			for (int x = 1; x <= Bezier.res; x++) {
				...
				new Line(vecs[0], vecs[1]).draw_line(g, fill, pane, xin, yin, location, true);
			}
	...
	public void generate_sprite(GameObject[] objects, String in) {
		...
		g.setColor(new Color(cols[4]));
		
		this.fill_shape(g, this);
		...
		System.out.println("TIME FOR SPRITE [" + this.id + "]: " + (double)(b - a) / 1000000);
		
		//System.exit(0);

		
		write_image("/Users/gregoryparamonau/Downloads/BEZIER_IMAGE.png", this.sprite);
	}
