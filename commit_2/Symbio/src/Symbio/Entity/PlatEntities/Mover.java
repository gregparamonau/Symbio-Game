...
public class Mover extends PlatEntity{
	...
	public void update() {
		...
		boolean player_on_mover = Rectangle.intersect(new Rectangle(this.pos.x, this.pos.y + 0.2, this.width + 0.8, this.height + 0.4), new Rectangle(Game.player));
	...
	public void generate_sprites(PlatEntity[] plats) {
		...
		for (int y = (int)(this.height / Camera.tile_size - 1); y >= 0; y--) {
			for (int x = 0; x<this.width / Camera.tile_size; x++) {
				this.sprites_num[count] = this.return_sprite_type(x, y, new Vector2(this.pos.x - this.width/2 + Camera.tile_size * (x + 0.5), this.pos.y - this.height/2 + Camera.tile_size * (y + 0.5)), plats);
				count++;
			}
		}
	}
	...
	public String toString() {
		String stops = "";
		for (int x = 4; x<this.nodes.length; x++) stops += this.nodes[x].x + " " + this.nodes[x].y + " ";
	...
	public static void create_props(JFrame temp, JTextField[] fields, JLabel[] labels) {
		...
		fields[1] = new JTextField(LevelCreator.plat_ents[LevelCreator.plat_ent_select[0]].stops.length + "");
		fields[1].setBounds(50, 100, 200, 25);
		fields[1].setVisible(true);
		temp.add(fields[1]);
		
		labels[1] = new JLabel("number of nodes");
