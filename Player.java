...
import Symbio.Rendering.Animation;
...
public class Player {
	...
	public static final int width = 40, height = 60;
	public static final int sprite_width = 100, sprite_height = 100;
	public Color fill = Color.DARK_GRAY;
	public static Color dash_color = Color.cyan;
	public Rectangle[] dash_trail = new Rectangle[5];
	public Vector2[] trail = new Vector2[200];
	
	//unlocked abilities
	public boolean jump_unlock = true, dash_unlock = true, wall_jump_unlock = true;
	//physics
	public boolean col_left, col_right, col_up, col_down, col;
	public Vector2 vel = new Vector2(0, 0);
	public double acceleration = 1.63, gravity = 1, friction = 1;
	...
	public static final int squash_death_threshold = 20;
	...
	public static int jump_time = 300;//time (ms) until max of jump
	public int jump_buffer, coyote_buffer;
	public boolean jumped;
	
	//wall jump
	static Vector2 wall_jump = new Vector2(25, 35);
	public boolean wall_jumped;
	public boolean wall_slide;
	static double wall_slide_speed = 6.6;
	...
	public double dash_dist, dash_length = 225;
	...
	public Vector2 swipe_horizontal = new Vector2(100, 80);
	public Vector2 swipe_vertical = new Vector2(80, 100);
	...
	public void update() {
		...
		this.move_player();
		this.last_move = Vector2.add(this.pos, Vector2.neg(temp));
		...
		//collision handling
		this.col_left = this.platforms_intersect(new Vector2(-1, 0)) || (this.plat_ent_intersect(new Vector2(-1, 0)) && this.swipe_cooldown == 0);
		this.col_right = this.platforms_intersect(new Vector2(1, 0)) || (this.plat_ent_intersect(new Vector2(1, 0)) && this.swipe_cooldown == 0);
		
		this.col_up = this.platforms_intersect(new Vector2(0, 1)) || (this.plat_ent_intersect(new Vector2(0, 1)) && this.swipe_cooldown == 0);
		this.col_down = this.platforms_intersect(new Vector2(0, -1)) || (this.plat_ent_intersect(new Vector2(0, -1)) && this.swipe_cooldown == 0);
		
		//if (this.col_left && this.col_right && this.col_up && this.col_down) this.respawn();
		
		this.wall_slide = (this.col_left || this.col_right) && (!this.col_down || !this.jump_unlock);
		this.col = this.col_left || this.col_right || this.col_up || this.col_down;
		
		if (this.col && !this.col_up) this.swipe_gravity = false;
		...
		if (this.last_move.y < 0 && !this.swipe_gravity) {
			this.friction = 1.5;
		}
		else if (Math.abs(this.last_move.y) < 5) {
			this.friction = 0.25;
		}
		...
		if (this.momentum.length() != 0) {
			...
			if (this.plat_ent_momentum_timer == 0 && (this.momentum.length() < 2 || (this.col && !this.jumped && !this.swipe_gravity))) {
		...
		if (this.jump_buffer > 0 && (this.col_down || this.col_left || this.col_right || this.on_hair || this.plat_ent_intersect(new Vector2(0, 0)))) {
			this.jump();
			this.jump_buffer = 0;
		}
	...
	public void jump() {
		...
		this.gravity = 1;
		this.jumped = true;
		if (this.dashing && !this.face_dir.equals(Vector2.up) && this.col_down) {
			this.end_dash("jump");
	...
	public void wall_jump(int in, boolean plat_ent_intersect, boolean jump_down) {
		...
		if (!this.wall_jumped) {
			...
			if (this.plat_ent_intersect_id != -1) {
				Game.current_room.plat_ents[this.plat_ent_intersect_id].transfer_player_momentum();
			}
			this.gravity = 1;
			this.wall_jumped = true;
		}
		else {
			Vector2 move = new Vector2((this.vel.x + this.last_dir * speed + this.momentum.x) * Game.speed_mult * 0.75, (this.vel.y + this.momentum.y) * Game.speed_mult);
			if (in == 0) move.x = this.vel.x;
			this.move(move);
			//check if intersect with any plat_ents
			if (plat_ent_intersect) plat_ent_intersect();
	...
	public void move(Vector2 in) {
		...
		//for 'y' axis
		if (in.y != 0) {
			for (int y = 0; y<Math.abs(in.y); y++) {
				this.pos.y += in.y/Math.abs(in.y);
				if (this.plat_ent_intersect(new Vector2(0, 0)) || this.platforms_intersect(new Vector2(0, 0))) {
					this.pos.y -= Utility.sign(in.y);
					break;
				}
			}
		}
		//for 'x' axis
		if (in.x != 0) {
			for (int x = 0; x<Math.abs(in.x); x++) {
				this.pos.x += in.x/Math.abs(in.x);
				if (this.plat_ent_intersect(new Vector2(0, 0)) || this.platforms_intersect(new Vector2(0, 0))) {
					this.pos.x -= Utility.sign(in.x);
					break;
				}
			}
		}
	...
	public void displace() {
		for (Platform plat: Game.current_room.platforms) {
			if (plat.type != 0 || !Rectangle.intersect(plat, new Rectangle(this))) continue;
			Rectangle temp = Rectangle.intersect_area(plat, new Rectangle(this));
			if (temp.height <= temp.width) {
				this.pos.y += Utility.sign(this.pos.y - plat.pos.y) * (temp.height + 1);
				break;
			}
			else if (temp.height > temp.width) {
				this.pos.x += Utility.sign(this.pos.x - plat.pos.x) * (temp.width + 1);
				break;
			}
		}
	}
	...
	//COLLISION
	public boolean platforms_intersect(Vector2 in) {
		for (Platform plat: Game.current_room.platforms) {
			if (plat == null || !Utility.on_screen_custom(plat, new Rectangle(Game.cam.pos.x, Game.cam.pos.y, Game.pane.getWidth() + 200, Game.pane.getHeight() + 200))) continue;
			if (Rectangle.intersect(plat, new Rectangle(this.pos.x + in.x, this.pos.y + in.y, width, height))) {
				return plat.collide_action();
			}
		}
		return false;
	}
	public boolean inside_platform() {
		for (Platform plat: Game.current_room.platforms) {
			if (plat.type != 0 || !Rectangle.intersect(plat, new Rectangle(this))) continue;
			Rectangle temp = Rectangle.intersect_area(plat, new Rectangle(this));
		...
		for (PlatEntity plat: Game.current_room.plat_ents) {
			if (!plat.collide_with(new Rectangle(this), true)) continue;
			Rectangle temp = Rectangle.intersect_area(plat, new Rectangle(this));
			if (Math.min(temp.width, temp.height) > squash_death_threshold) return true;
		}
		return false;
	}
	public boolean plat_ent_intersect() {
		if (this.grab && this.on_hair && !this.jumped) {
			this.move(Game.current_room.plat_ents[this.plat_ent_intersect_id].snap_player_to_plat_ent());
		}
		this.on_hair_last_frame = this.on_hair;
		this.on_hair = false;
		for (int x = 0; x<Game.current_room.plat_ents.length; x++) {
			if (!Utility.on_screen_custom(Game.current_room.plat_ents[x], new Rectangle(Game.cam.pos.x, Game.cam.pos.y, Game.pane.getWidth() + 200, Game.pane.getHeight() + 200))) continue;
			if (Game.current_room.plat_ents[x].collide_with(this)) {
				Game.current_room.plat_ents[x].collision_action();
				return true;
			}
		}
		return false;
	}
	public boolean plat_ent_intersect(Vector2 in) {
		for (int x = 0; x<Game.current_room.plat_ents.length; x++) {
			if (!Utility.on_screen_custom(Game.current_room.plat_ents[x], new Rectangle(Game.cam.pos.x, Game.cam.pos.y, Game.pane.getWidth() + 200, Game.pane.getHeight() + 200))) continue;
			if (Game.current_room.plat_ents[x].collide_with(new Rectangle(this.pos.x + in.x, this.pos.y + in.y, width, height), true)) {
				return true;
			}
		}
		return false;
	...
	public void end_dash(String in) {
		...
		else if (in.equals("dash")){
			this.momentum = Vector2.mult(this.dash_dir, speed * (double)this.dash_length/100);
		}
		else if (in.equals("hair")) {
			this.momentum = Vector2.mult(this.dash_dir, speed * (double)this.dash_length/100);
		}
		this.dash_dir = new Vector2();
	}
	public void start_debug_trail() {
		for (int x = 0; x<this.trail.length; x++) this.trail[x] = new Vector2(this.pos);
	}
	public void update_trail() {
		for (int x = this.trail.length - 1; x > 0; x--) {
			this.trail[x] = this.trail[x - 1];
		}
		this.trail[0] = new Vector2(this.pos);
	}
	public void draw_trail(Graphics g) {
		for (Vector2 temp: this.trail) temp.draw_node(g, Game.pane, Game.cam.pos.x, Game.cam.pos.y, "game", new Color(255, 0, 255, 128));
	}
	public void start_trail(int q) {
		this.dash_trail = new Rectangle[q];
		for (int x = 0; x<this.dash_trail.length; x++) {
			this.dash_trail[x] = new Rectangle(this);
	...
	public void add_to_dash_trail() {
		...
		temp[this.dash_trail.length] = new Rectangle(this);
	...
	public void respawn() {
		//TODO: finish this
		this.set_position(Game.player_xpos, Game.player_ypos);
	...
	public void draw_player(Graphics g) {
		...
		(new Rectangle(this)).draw_with_sprite(g, Game.pane, Game.cam.pos.x, Game.cam.pos.y, this.sprite_to_play(), "game");
