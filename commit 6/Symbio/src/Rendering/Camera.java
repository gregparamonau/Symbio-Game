...
public class Camera {
	...
	public void draw_view(Graphics2D g, int delta_time) {
		...
		for (int x = 0; x<Game.current_room.objects.length; x++) Game.current_room.objects[x].draw_object(g, Start.pane, this.pos.x, this.pos.y, "game");
		...
		PlayerRender.draw_player(Game.player, g);
		
		if (Game.debug_mode) {
			this.draw_debug_menu(g, delta_time, Game.player);
			g.setColor(Color.magenta);
			g.drawRect(0, 0, Start.pane.getWidth() / pixel_size, Start.pane.getHeight() / pixel_size);
			
			new Vector2(Game.player.respawn_point).draw_node(g, Start.pane, this.pos.x, this.pos.y, "game", Color.green);
			new Rectangle(0, 0, 16 * tile_size, 16 * tile_size).draw_border(g, Start.pane, this.pos.x, this.pos.y, "game");
		}
	...
	public void draw_HUD(Graphics g) {
		for (int x = 0; x<Game.player.health; x++) {
			new Rectangle(HUD_offset.x + this.pos.x + (x * 2 + 1) * tile_size, HUD_offset.y + this.pos.y - tile_size, 0, 0).draw_with_sprite(g, Start.pane, this.pos.x, this.pos.y, this.health_sprite.sprites[0], "game");
		}
	}
	...
	public void draw_debug_menu(Graphics2D g, int delta_time, Player in) {
		...
		g.drawString("Bunny Hop: " + in.momentum_keep_buffer, width, width * 7);
	...
	public void move() {
		this.pos.x = (int)Utility.clamp(Utility.move_toward(this.pos.x, Game.player.pos.x + Game.player.last_dir * look_ahead, (int)Utility.clamp(Math.abs(this.pos.x - (Game.player.pos.x + Game.player.last_dir * look_ahead))/10, 1, 100000)), this.min_x + Start.pane.getWidth() / 2 / Camera.pixel_size, this.max_x - Start.pane.getWidth() / 2 / Camera.pixel_size);
