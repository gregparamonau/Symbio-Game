...
public class Slope extends GameObject{
	...
	public void give_dash_jump_momentum() {
		Vector2 temp = Vector2.sub(this.b, this.a);
		
		Vector2 out = Vector2.scale_to_length(temp, Game.player.momentum.length() * dash_jump_mom_mult * Vector2.dot(temp, Game.player.momentum) * Utility.sign(Vector2.dot(temp, new Vector2(Game.player.last_dir, 0))));
		
		Game.player.momentum = new Vector2(out);
	}
	public void displace_player(int direction) {
		
		long a = System.nanoTime();
		
		if (!this.collide_with(Game.player, false)) return;
		//direction: 0 = any, 1 = vertical, 2 = horizontal
		Vector2 temp = Line.find_node_on_line(new Line(this.a, this.b), Game.player.pos);
		
		Vector2[] nodes = {
				new Vector2(Game.player.pos.x + Game.player.width / 2, Game.player.pos.y + Game.player.height / 2),
				new Vector2(Game.player.pos.x - Game.player.width / 2, Game.player.pos.y + Game.player.height / 2),
				new Vector2(Game.player.pos.x - Game.player.width / 2, Game.player.pos.y - Game.player.height / 2),
				new Vector2(Game.player.pos.x + Game.player.width / 2, Game.player.pos.y - Game.player.height / 2),
		};
		...
		Vector2 offset = Vector2.sub(Game.player.pos, nodes[place]);
		...
		if (direction == 0) {
			if (Math.abs(x - nodes[place].x) >= Math.abs(y - nodes[place].y)) Game.player.pos.y += y - nodes[place].y;
			if (Math.abs(x - nodes[place].x) < Math.abs(y - nodes[place].y)) Game.player.pos.x += x - nodes[place].x;
		}
		if (direction == 1) {
			Game.player.pos = Vector2.add(new Vector2(nodes[place].x, y), offset);
		}
		if (direction == 2) {
			Game.player.pos = Vector2.add(new Vector2(x, nodes[place].y), offset);
		}
