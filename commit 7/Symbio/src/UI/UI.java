...
public class UI {
	...
	public static void main_menu() {
		...
		Start.count = 0;
		do {
			current_mouse_pos = convert_mouse_pos(current_mouse_pos(), Start.cam);
		...
		if (Start.choice == 0) Game.game();
		if (Start.choice == 1) {
			Start.pane.setSize(Start.pane.getWidth() * Camera.tile_size, Start.pane.getHeight() * Camera.tile_size);
	...
	public static void update_pause_menu() {
		current_mouse_pos = convert_mouse_pos(current_mouse_pos(), Game.cam);
	}
	...
	public static void draw_main(Graphics g) {
		...
		current_mouse_pos.draw_node(g, Start.pane, 0, 0, "Start", Color.red);
	...
	public static void main_init() {
		...
		buttons = new Button[] {
				...
				new Button(corner_offset.x + button_dimensions.x / 2, - button_dimensions.y - button_spacing - padding / 2, button_dimensions.x, button_dimensions.y, "editor ", "level_edit_function"),
				new Button(corner_offset.x + button_dimensions.x / 2, - 2 * (button_dimensions.y + button_spacing) - padding / 2, button_dimensions.x, button_dimensions.y, "options ", "main_options_function"),
				new Button(corner_offset.x + button_dimensions.x / 2, - 3 * (button_dimensions.y + button_spacing) - padding / 2, button_dimensions.x, button_dimensions.y, "quit ", "main_quit_function")
		};
	}
	...
	public static Vector2 convert_mouse_pos(Vector2 in, Camera cam) {
		return new Vector2(cam.pos.x - (Start.pane.getWidth() / 2 - in.x) / Camera.pixel_size, cam.pos.y - (in.y - Start.pane.getHeight() / 2) / Camera.pixel_size);
