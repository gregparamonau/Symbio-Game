...
public class Player {
	...
	public static final int width = 4, height = 12;
	...
	//unlocked abilities
	public boolean jump_unlock = true, dash_unlock = true, wall_jump_unlock = true, swipe_unlock = false, dash_bounce_unlock = false;
	...
	public void update() {	
		...
		//collision handling
		this.col_left = this.platforms_intersect(new Vector2(-0.2, 0)) || (this.plat_ent_intersect(new Vector2(-0.2, 0)) && this.swipe_cooldown == 0);
		this.col_right = this.platforms_intersect(new Vector2(0.2, 0)) || (this.plat_ent_intersect(new Vector2(0.2, 0)) && this.swipe_cooldown == 0);
		
		this.col_up = this.platforms_intersect(new Vector2(0, 0.2)) || (this.plat_ent_intersect(new Vector2(0, 0.2)) && this.swipe_cooldown == 0);
	...
	public void move(Vector2 in, int b) {
		in = Vector2.mult(in, 0.2);
		
		for (int i = 0; i<in.x%width; i++) {
			this.pos.x += Utility.sign(in.x) * width;
		}
	}
	...
	public void move(Vector2 in) {
		...
		if (in.x != 0) {
			for (int x = 0; x<Math.abs(in.x) * 5; x++) {
				...
				if (this.plat_ent_intersect(new Vector2(0, 0)) || this.platforms_intersect(new Vector2(0, 0))) {
					if (!this.plat_ent_intersect(new Vector2(0, 0.2)) && !this.platforms_intersect(new Vector2(0, 0.2))) {
						this.pos.y += 0.2;
						continue;
					}
					this.pos.x -= Utility.sign(in.x) * 0.2;
	...
	public boolean plat_ent_intersect() {
		if (this.grab && this.on_hair && !this.jumped) {
			Vector2 move = Game.current_room.plat_ents[this.plat_ent_intersect_id].snap_player_to_plat_ent();
			if (move.length() > 1) this.move(move);
		}
		this.on_hair_last_frame = this.on_hair;
		if (!this.grab || this.jumped) this.on_hair = false;
		for (int x = 0; x<Game.current_room.plat_ents.length; x++) {
	...
	public void draw_player(Graphics g) {
		...
		new Rectangle(this.pos.x, this.pos.y - 0.01, 0, 0).draw_with_sprite(g, Game.pane, Game.cam.pos.x, Game.cam.pos.y, this.sprite_to_play(), "game");
		...
		if (Game.debug_mode) new Rectangle(this).draw_border(g, Game.pane, Game.cam.pos.x, Game.cam.pos.y, "game");
