...
public class Circle extends GameObject{
	...
	public void displace_player(int direction) {
		if (!Rectangle.intersect_circle(Game.player, pos, this.radius)) return;
		
		Vector2 clamp = new Vector2(Utility.clamp(this.pos.x, Game.player.pos.x - Game.player.width / 2, Game.player.pos.x + Game.player.width / 2), Utility.clamp(this.pos.y, Game.player.pos.y - Game.player.height / 2, Game.player.pos.y + Game.player.height  / 2));
		
		Vector2 dist = Vector2.sub(clamp, this.pos);
		
		Vector2 offset = Vector2.sub(Vector2.scale_to_length(dist, this.radius), dist);
		
		Game.player.pos.add(offset);
	}
	
	public void give_dash_jump_momentum() {
		
		Vector2 temp = Vector2.perp(Vector2.sub(Game.player.pos, this.pos), Game.player.last_dir);
		
		Game.player.momentum = Vector2.scale_to_length(temp, Game.player.momentum.length() * dash_jump_mom_mult * Vector2.dot(temp, Game.player.momentum));
	}
