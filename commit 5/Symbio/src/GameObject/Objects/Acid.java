...
public class Acid extends GameObject{
	...
	public static Acid default_acid(Vector2 loc) {
		return new Acid(loc.x, loc.y, LevelEditor.grid_size, LevelEditor.grid_size, -1, sprite_name_default, 0);
	}
	public void update() {
	...
	void update_bubbles() {
		...
		for (int x = 0; x<this.bubbles.length; x++) {
			...
			if (this.bubbles[x].y > this.height / 2 - Camera.tile_size / 2 && this.count[x] == 14) {
				this.bubbles[x].y = - Math.random() * this.height / 2 + Camera.tile_size / 2;
				this.count[x] = 0;
				continue;
			}
			
			if (this.count[x] > 4 && this.bubbles[x].y < this.height / 2 - Camera.tile_size / 2) this.count[x] = 5 + (this.count[x] + 1) % 5;
			else this.count[x] ++;
		}
	...
	void draw_bubbles(Graphics g, JPanel pane, double xin, double yin, String location) {
		for (int x = 0; x<this.bubbles.length; x++) {
			new Rectangle(this.bubbles[x].x + this.pos.x, this.bubbles[x].y + this.pos.y, 0, 0).draw_with_sprite(g, pane, xin, yin, this.return_sprite(this.count[x]), location);
		}
	}
