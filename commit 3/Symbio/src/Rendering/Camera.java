...
public class Camera {
	...
	public void draw_view(Graphics2D g, int delta_time) {
		...
		if (Game.debug_mode) {
			...
			new Vector2(Game.player_respawn).draw_node(g, Game.pane, this.pos.x, this.pos.y, "game", Color.green);
	...
	public void draw_debug_menu(Graphics2D g, int delta_time, Player in) {
		...
		g.drawString("Last Move: " + in.last_move.round(3), width, width*5);
		g.drawString("Momentum Timer: " + in.plat_ent_momentum_timer, width, width*6);
		g.drawString("Gravity: " + in.gravity, width, width*7);
